...
const LanguageChart = forwardRef(({ languages }, ref) => {
  if (!languages || Object.keys(languages).length === 0) return null;
  const COLORS = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b"];
  const data = Object.entries(languages).map(([lang, bytes]) => ({ name: lang, value: bytes }));

  return (
    <div ref={ref} style={{ width: 220, height: 220, margin: "auto" }}>
      <h3 className="text-base font-semibold text-center mb-2">Language Distribution</h3>
      <ResponsiveContainer width="100%" height="85%">
        <PieChart>
          <Pie
      ...
      {/* Small legend below chart */}
      <div className="flex flex-wrap justify-center gap-2 mt-2 mb-6 text-xs">
